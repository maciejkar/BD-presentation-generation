...
def true_time_to_time(scrap_time : str , mul : float ):  
    ...
    4:47  zmienia na 6  dla stałej wynoszącej 1.2"""
    time = scrap_time.strip()
    return str(round((int(time[:-3]) + int(time[-2:])/60) * mul )) + ' h'

def scrap(path,setting_path = '../../settings.json'):
    ...
    heightest_point =soup.find('div', class_='ts-user-route__data-inline-item').get_text()[25:].replace('\n','').replace('\t','').replace(',',', ')
    index = heightest_point.find('m n.p.m')
    max_height = int(heightest_point[index-5:index -1])
